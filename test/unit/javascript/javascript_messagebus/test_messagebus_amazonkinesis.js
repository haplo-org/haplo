...
TEST(function() {
    ...
    messagebus_test_kinesis.db.info.select().deleteAll();

    var testValue = Date.now() % 10000000;
    var testMsg = Bus.message().body({value:testValue});
    testMsg.send();

    TEST.assert_equal(0, messagebus_test_kinesis.db.info.select().count());

    // Use callback to trigger delivery
    $host._testCallback("deliverAmazonKinesisMessages");

    TEST.assert_equal(1, messagebus_test_kinesis.db.info.select().count());

    // Check delivery report now it's been sent to Kinesis
    var report = JSON.parse(messagebus_test_kinesis.db.info.select()[0].info);
