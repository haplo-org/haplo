...
(function() {
    ...
    O.editor.decode = function(encoded, object) {
        if(!(object.$kobject instanceof $StoredObjectInterface) || !(object.firstType())) {
            throw new Error("O.editor.decode() must be passed a StoreObject which has a type attribute.");
        }
        $StoredObjectInterface.clientSideEditorDecode(encoded, object);
        return object;
    };
    ...
    O.editor.encode = function(object) {
        ...
        return $StoredObjectInterface.clientSideEditorEncode(object);
    };
