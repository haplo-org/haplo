...
(function() {
    ...
    $DbObject.$defineSetter = {
        ...
        // ---- JSON
        "json": function(fieldName, nullNotAllowed) {
            return function(value) {
                if(value === null) { if(nullNotAllowed) { throw new Error(fieldName+" cannot be null");} }
                else if(typeof(value) !== 'object')  { throw new Error(fieldName+" must be a JSON compatible object"); }
                if(this.$changes == undefined) { this.$changes = {}; }
                var serialised = (value === null) ? null : JSON.stringify(value);
                this.$changes[fieldName] = serialised;
                this.$values[fieldName] = serialised;
                this.$values[fieldName+'_deserialised'] = undefined;    // uncache
            };
        },
        // ---- NUMERIC
        "numeric": function(fieldName, nullNotAllowed) {
            return function(value) {
                if(value === null) { if(nullNotAllowed) { throw new Error(fieldName+" cannot be null");} }
                else if(!(value instanceof $BigDecimal))  { throw new Error(fieldName+" must be a BigDecimal object"); }
                if(this.$changes == undefined) { this.$changes = {}; }
                this.$changes[fieldName] = value;
                this.$values[fieldName] = value;
            };
...
$DbObject.$makeFactoryFunction = function(tableDefinition, fields, initialiserOrMethods) {
    ...
    _.each(fields, function(defn, fieldName) {
        var fieldType = defn.type;
        var setterDefiner = $DbObject.$defineSetter[fieldType];
        if(setterDefiner === null) {
            throw new Error("When defining a database table, field type '"+defn.type+"' is unknown.");
        }
        var nullNotAllowed = ("nullable" in defn) ? (!defn.nullable) : true;
        if(fieldType === "link") {
            ...
            if(fieldType === "user") {
                ...
                // JSON needs automatic deserialisation
                var deserialisedKey = fieldName+"_deserialised";
                DbObject.prototype.__defineGetter__(fieldName, function() {
                    var deserialised = this.$values[deserialisedKey];
                    if(undefined !== deserialised) { return deserialised; }
                    var serialised = this.$values[fieldName];
                    if(serialised === null) {
                        deserialised = null;
                    } else {
                        deserialised = JSON.parse(serialised);
                    }
                    this.$values[deserialisedKey] = deserialised;
                    return deserialised;
                });
            } else {
