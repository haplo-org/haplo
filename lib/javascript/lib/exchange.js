...
// Also created directly by std_web_publisher
...
// Also created directly by std_web_publisher & via hook by FileController
$Exchange.$Request = function(details) {
    _.extend(this, details);
};
$Exchange_Request = $Exchange.$Request; // for accessibility by hook argument constructor
...
(function() {
    ...
    _.extend($Exchange.prototype, {
        // Allow plugins which extend other plugins to fully control request handling
        setResponsiblePlugin: function(plugin) {
            this.$plugin = plugin;
        },
        ...
        render: function(view, templateName) {
            ...
            } else if(templateName instanceof $HaploTemplate) {
            ...
            if(template.kind == "html") {
                ...
                if(view.layout == undefined) {
                    ...
                    if(view.layout.indexOf("std:") === 0) {
                        ...
                        console.log("Use of plugin templates for layouts is deprecated. Explicitly specify a layout template and use yield() to render contents instead.");
        ...
        renderIntoSidebar: function(view, templateName) {
            if(!view) { view = {}; }
            if(!templateName) {
                throw new Error("Template name not specified to renderIntoSidebar() function call.");
            }
            var template = (templateName instanceof $HaploTemplate) ? templateName : this.$plugin.template(templateName);
            if(template.kind !== "html") {
                throw new Error("Template "+templateName+" is not an HTML template");
            }
            $host.addRightContent(template.render(view));
        },
    ...
    $Exchange.prototype.__defineGetter__("continuation", function() {
        var c = this.$continuation;
        if(!c) {
            this.$continuation = c = $host.fetchRequestContinuation();
            this.$continuation._setExchange(this);
        }
        return c;
    });
    $Exchange.prototype.$continuationHasSuspendedThisRequest = function() {
        this.response.body = '$SUSPENDED_BY_CONTINUATION'; // contents ignored, just using a string to make this obvious
        this.response.statusCode = -11111;  // magic status code
    };
    O.resumeRequestWithContinuationIdentifier = function(identifier, attributes) {
        $RequestContinuation.resumeContinuationByIdentifierIfRegistered(identifier, attributes ? attributes : null);
    };
