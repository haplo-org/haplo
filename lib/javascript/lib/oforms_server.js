...
var standardTemplates = {
    ...
    'oforms:default:display': '{{#if sectionHeading}}<h2>{{sectionHeading}}</h2>{{/if}}{{#rows}}{{#if ../isRepeatingSection}}<div class="oforms-repetition{{#if ../../hasMultipleElements}} oforms-has-multiple{{/if}}">{{/if}}{{#elements}}<div class="oforms-display-row control-group">{{#if label}}<label class="control-label">{{label}}</label>{{/if}}<div class="controls">{{{html}}}</div></div>{{/elements}}{{#if ../isRepeatingSection}}</div>{{/if}}{{/rows}}',
...
makeElementType("boolean", {
    ...
    _initElement: function(specification, description) {
        this._trueLabel  = escapeHTML(specification.trueLabel  || 'Yes');
        this._falseLabel = escapeHTML(specification.falseLabel || 'No');
