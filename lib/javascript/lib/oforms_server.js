...
var getByPathOrExternal = function(context, x, instance) {
    var path = x.path;
    if(path) {
        return getByPath(context, path);
    } else if("externalData" in x) {
        return instance._externalData[x.externalData];
    }
};
...
var standardTemplates = {
    'oforms:default': '{{#unless rowsOnly}}{{#if sectionHeading}}<h2>{{sectionHeading}}</h2>{{/if}}<div class="oforms-append{{#if class}} {{class}}{{/if}}"{{#if id}} id="{{id}}"{{/if}}{{#if guidanceNote}} data-oforms-note="{{guidanceNote}}"{{/if}}>{{{extraTopUI}}}{{/unless}}{{#rows}}{{#if ../isRepeatingSection}}<div class="oforms-repetition{{#if ../../hasMultipleElements}} oforms-has-multiple{{/if}}">{{/if}}{{#elements}}<div{{#if uniqueName}} data-uname={{uniqueName}}{{/if}} class="oforms-row control-group{{#if validationFailure}} error{{/if}}">{{#if label}}<label class="control-label">{{label}}{{#if required}}<span class="oforms-required">&nbsp;*</span>{{/if}}</label>{{/if}}{{#if explanationHTML}}<div class="oforms-explanation">{{{explanationHTML}}}</div>{{/if}}<div class="controls">{{{html}}}{{#if validationFailure}}<div class="oforms-error-message help-block">{{validationFailure.message}}</div>{{/if}}</div></div>{{/elements}}{{#if ../allowDelete}}<a href="#" class="oforms-delete-btn btn btn-small" title="Delete this section"><i class="icon-minus-sign"></i>Delete</a>{{/if}}{{#if ../isRepeatingSection}}</div>{{/if}}{{/rows}}{{#unless rowsOnly}}</div>{{#if allowAdd}}<div><a href="#" class="oforms-add-btn btn btn-small{{#if displayingMaximumRows}} oform-add-btn-disabled{{/if}}" title="Add another section"><i class="icon-plus-sign"></i>Add another</a></div>{{/if}}{{/unless}}',
    'oforms:default:display': '{{#if sectionHeading}}<h2>{{sectionHeading}}</h2>{{/if}}{{#rows}}{{#if ../isRepeatingSection}}<div class="oforms-repetition{{#if ../../hasMultipleElements}} oforms-has-multiple{{/if}}">{{/if}}{{#elements}}<div class="oforms-display-row control-group"{{#if uniqueName}} data-uname={{uniqueName}}{{/if}} data-order="{{orderingIndex}}">{{#if label}}<label class="control-label">{{label}}</label>{{/if}}<div class="controls">{{{html}}}</div></div>{{/elements}}{{#if ../isRepeatingSection}}</div>{{/if}}{{/rows}}',
    ...
    'oforms:grid:row': '{{#rows}}{{#elements}}<td{{#if uniqueName}} data-uname={{uniqueName}}{{/if}}>{{#if validationFailure}}<span class="oforms-error">{{/if}}{{{html}}}{{#if required}}<span class="oforms-required"> *</span>{{/if}}{{#if validationFailure}}</span><br><span class="oforms-error-message">{{validationFailure.message}}</span>{{/if}}</td>{{/elements}}{{/rows}}',
...
var evaluateConditionalStatement = function(conditionalStatement, context, instance) {
    ...
    var check = function(statement) {
        ...
        switch(statement.operation) {
            case "defined":
                r = (getByPathOrExternal(context, statement, instance) !== undefined);
                break;
            case "not-defined":
                r = (getByPathOrExternal(context, statement, instance) === undefined);
                break;
            case "=": case "==": case "===":
                r = (getByPathOrExternal(context, statement, instance) === statement.value);
                break;
            case "!=": case "!==":
                r = (getByPathOrExternal(context, statement, instance) !== statement.value);
                break;
            ...
            case ">=":
                ...
                break;
            case "contains":
                r = ecsGetContains(context, statement, instance);
                break;
            case "not-contains":
                r = !ecsGetContains(context, statement, instance);
                break;
            case "minimum-count": 
                pathValue = getByPathOrExternal(context, statement, instance);
...
var ecsGetContains = function(context, statement, instance) {
    var pathValue = getByPathOrExternal(context, statement, instance);
    if(_.isArray(pathValue)) { // only makes sense for multiples
        ...
        if(renderForm) {
            ...
            if(vc) {
                output.push(
                    ...
                    (vc.unit === "character") ? 'Characters' : 'Words',
                    ': <span></span> '
                );
                if(vc.limitText) {
                    ...
                    var limits = [];
                    if(vc.min) { limits.push('min '+vc.min); }
                    if(vc.max) { limits.push('max '+vc.max); }
                    if(limits.length) {
            ...
            if(countFailure) {
                ...
                if(m.specifiedMessage in vc) {
                    ...
                    validationResult._failureMessage = m.prefix + vc[m.count] + ' ' +
                        ((vc.unit === "character") ? 'Characters' : 'Words').toLowerCase() +
                        ' ' + m.suffix;
                }
            }
        }
...
makeElementType("date", {
    ...
    _decodeValueFromFormAndValidate: function(instance, nameSuffix, submittedDataFn, validationResult, context) {
        if(this._validationRelativeTo) {
            var relativeDate = getByPathOrExternal(context, this._validationRelativeTo, instance);
        ...
        if(validateDate(dateStr)) {
            ...
            validationResult._failureMessage = this._validationFailureMessage || 'Date is not within allowed range';
        }
        if(dateStr) {
            // If the user entered something, tell them it was invalid.
            validationResult._failureMessage = validationResult._failureMessage || 'Not a valid date';
