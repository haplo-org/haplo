...
// TODO: Should this interface be called a 'text' type, as it looks like it'll be used for so much more?
...
(function() {
    ...
    var hObjectTextValueReplaceMatchingRefImpl = function(response, type, value, ref, replacementRef) {
        // Does this plugin implement this callback?
        var info = this.$textTypes[type];
        if(undefined === info) { return; }
        var implementation = info.implementation;
        if("replaceMatchingRef" in implementation) {
            // Parse the value as JSON
            value = JSON.parse(value);
            var output = implementation.replaceMatchingRef(value, ref, replacementRef);
            if(output !== undefined && output !== null) {
    ...
    $Plugin.prototype.implementTextType = function(type, description, implementation) {
        if(undefined == this.$textTypes) {
            ...
            this.hObjectTextValueReplaceMatchingRef = hObjectTextValueReplaceMatchingRefImpl;
        }
    ...
    // ----------------------------------------------------------------------------------------------------------------------
    // Efficient check that a value is a given plugin defined text type

    O.isPluginTextValue = function(value, type) {
        return (value instanceof $KText) && (value._pluginDefinedTextType === type);
    };
