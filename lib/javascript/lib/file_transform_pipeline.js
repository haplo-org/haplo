...
(function() {
    ...
    _.extend(Pipeline.prototype, {
        ...
        _identifierForDownloadableFile: function(name, filename, view) {
            this._ensureNotExecuted();
            checkName(name);
            var identifier = O.security.random.identifier();
            this.downloads.push([name, filename, identifier, (view || {})]);
            return identifier;
        },
        ...
        urlForOutput: function(name, filename) {
            return O.application.url +
                "/do/generated/file/" +
                this._identifierForDownloadableFile(name, filename) +
                "/"+filename.replace(/[^a-zA-Z0-9_\.-]+/,'_');
        },

        urlForOuputWaitThenDownload: function(name, filename, view) {
            return "/do/generated/download/" +
                this._identifierForDownloadableFile(name, filename, view) +
                "/"+filename.replace(/[^a-zA-Z0-9_\.-]+/,'_');
        },

        viewToWaitForOutput: function(name, filename) {
            return {
                identifier: this._identifierForDownloadableFile(name, filename),
                filename: filename
            };
        },
