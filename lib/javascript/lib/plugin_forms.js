...
(function() {
    ...
    var FormInstanceWrapper = O.$private.oForms.fiw = function(plugin, instance, localeId) {
        this.plugin = plugin;
        this.instance = instance;
        this.document = instance.document;
        this.$instanceLocaleId = localeId;
    };
    _.extend(FormInstanceWrapper.prototype, {
        ...
        renderForm: function() {
            ...
            if(description.requiresBundle) {
                ...
                var bundleUrlPath = '/api/oforms/bundle'+$host.pluginStaticDirectoryUrl(this.plugin.$pluginName)+'/'+
                        this.$instanceLocaleId+'/'+
                        description.formId+'/'+
                        uid;
    ...
    _.extend(FormDescriptionWrapper.prototype, {
        ...
        _descriptionForLocale: function(locale) {
            var descriptionForLocale;
            if(locale.defaultForPlugin) {
                descriptionForLocale = this.description;
            } else {
                var translatedDescriptions = this.$translatedDescriptions;
                if(!translatedDescriptions) { translatedDescriptions = this.$translatedDescriptions = {}; }
                descriptionForLocale = translatedDescriptions[locale.id];
                if(!descriptionForLocale) {
                    // Make a copy of the specification and translate all the strings through the locale
                    var translatedSpecification = JSON.parse(JSON.stringify(this.specification));
                    replaceAllTranslatableStringsInFormSpecification(translatedSpecification, locale.text("form"));
                    descriptionForLocale = oForms.createDescription(translatedSpecification, this.plugin.$formDelegate);
                    descriptionForLocale.$haploPlugin = this.plugin;    // for custom elements
                    // Cache translated description
                    translatedDescriptions[locale.id] = descriptionForLocale;
                }
            }
        ...
        _instanceWithoutReplace: function(document) {
            ...
            var locale = this.plugin.locale();
            return new FormInstanceWrapper(
                this.plugin,
                this._descriptionForLocale(locale).createInstance(document),
                locale.id
            );
        },
        ...
        _getBundleJavaScriptResponse: function(localeId) {
            this._ensureDescriptionCreated();
            ...
            var locale = this.plugin.locale(localeId);
            var bundle = this._descriptionForLocale(locale).generateBundle();
            return [
    ...
    const TRANSLATED_PROPERTIES_FORM = ["formTitle", "formTitleShort"];
    const TRANSLATED_PROPERTIES_ELEMENT = [
        "label", "explanation", "placeholder", "guidanceNote", "inlineGuidanceNote", // common
        "trueLabel", "falseLabel", "notConfirmedMessage", // boolean
        "prompt", // choice
        "heading", // sections
        "text", // static
        "validationFailureMessage" // text
    ];
    var replaceAllTranslatableStringsInFormSpecification = O.$private.oForms.i18nrats = function(specification, text) {
        let translateObject = function(o, props) {
            Object.keys(o).forEach(function(k) {
                if(-1 !== props.indexOf(k)) {
        ...
        // Top level of form
        translateObject(specification, TRANSLATED_PROPERTIES_FORM);
        // Elements
        let translateContainedElements = function(x) {
            if(x.elements) {
                x.elements.forEach(function(e) {
                    translateObject(e, TRANSLATED_PROPERTIES_ELEMENT);
                    translateContainedElements(e);
                    // Translate embedded choices
                    if("choices" in e && _.isArray(e.choices)) {
                        e.choices.forEach(function(c) {
                            if(_.isArray(c) && c.length === 2 && typeof(c[1]) === "string") {
                                c[1] = text[c[1]];
                            }
                        });
                    }
                    if("radioClusters" in e && _.isArray(e.radioClusters)) {
                        e.radioClusters.forEach(function(c) {
                            if(c.label)       { c.label       = text[c.label]; }
                            if(c.explanation) { c.explanation = text[c.explanation]; }
                        });
                    }
                    // Translate rich text guidance notes
                    if(typeof(e.inlineGuidanceNote) === "object" && e.inlineGuidanceNote.note) {
                        e.inlineGuidanceNote.note = text[e.inlineGuidanceNote.note];
                    }
                });
            }
        };
        translateContainedElements(specification);
    };
    ...
    FormDescriptionWrapper.prototype.__defineGetter__("formTitle", function() {
        var locale = this.plugin.locale();
        var titleKey = "$formTitle."+locale.id;
        var t = this[titleKey];
        if(!t) {
            t = this[titleKey] = formsTextTranslate(
                locale.text("form")[
                    this.specification.formTitle || '????'
                ]
            );
        }
        return t;
    ...
    FormDescriptionWrapper.prototype.__defineGetter__("formTitleShort", function() {
        var locale = this.plugin.locale();
        var titleKey = "$formTitleShort."+locale.id;
        var t = this[titleKey];
        if(!t) {
            t = this[titleKey] = formsTextTranslate(
                locale.text("form")[
                    this.specification.formTitleShort || this.specification.formTitle || '????'
                ]
            );
        }
        return t;
    ...
    var hPlatformInternalOFormsBundleImpl = O.$private.oForms.hpiofbi = function(response, pluginName, formId, localeId) {
        var wrappedDescription;
        if(     pluginName === this.$pluginName &&
                this.$formLookupById &&
                (wrappedDescription = this.$formLookupById[formId]) ) {
            response.bundle = wrappedDescription._getBundleJavaScriptResponse(localeId);
        }
    };
    ...
    // ----------------------------------------------------------------------------------------------------------------------
    // i18n for oForms strings

    var formsI18nTextLookup = $Locale.__i18nGetTranslationInPlatformTextForCurrentLocale;
    oForms.i18nTextLookup(formsI18nTextLookup);
    ...
    oForms._makeElementType("render-template", {
        ...
        _initElement: function(specification, description) {
            this._pluginName = specification.plugin;
            this._template = specification.template;
            this._view = specification.view;
        },

        _pushRenderedHTML: function(instance, renderForm, context, nameSuffix, validationFailure, output) {
            var formPlugin = instance.description.$haploPlugin;
            var plugin = formPlugin;
            if(this._pluginName) {
                var n = this._pluginName;
                plugin = _.find($registry.plugins, function(p) { return p.pluginName === n; });
                if(!plugin) {
                    throw new Error("Unknown plugin in render-template: "+this._pluginName);
                }
            }
            var template = plugin.template(this._template);
            output.push(template.render({
                formPlugin: formPlugin,
                isRenderingForm: renderForm,
                view: this._view,
