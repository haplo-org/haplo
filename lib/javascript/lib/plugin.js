...
(function() {
    ...
    var ALLOWED_HTTP_METHODS = {GET:true, POST:true};
    ...
    _.extend($Plugin.prototype, {
        ...
        implementService: function(name, serviceFunction) {
            return O.$private.$registerService(name, serviceFunction, this, this.$pluginName);
        },
        ...
        onInstall: function() {
            // NOTE: Never actually called because KONEISHost only looks in the plugin object, not the prototype
        },
        ...
        handleRequest: function(method, path) {
            ...
            } catch(e if e instanceof O.$private.$stopError) {
    ...
    $Plugin.$callOnLoad = function() {
        ...
        O.$private.$useRegisteredServicesBeforeOnLoad();
    ...
    $Plugin.$callAllHooks = function(hookName, response) { // called with other arguments for plugin hook
        var plugins = $registry.plugins, n = plugins.length, i, plugin;
        for(i = 0; i < n; i++) {
            plugin = plugins[i];
            if(plugin[hookName] !== undefined) {
                $host.setLastUsedPluginName(plugin.$pluginName); // for blaming plugins when errors occur
                plugin[hookName].apply(plugin, Array.prototype.slice.call(arguments, 1));
                if(response.shouldStopChain()) { return response; }
            }
        }
        return response;
    };
