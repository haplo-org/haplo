...
(function() {
    ...
    template("std:new_object_editor", function(view) {
        return $host.renderRTemplate("new_object_editor",
            vObj(view,"templateObject"), v(view,"successRedirect"), v(view,"readOnlyAttributes"));
    });
    // --------
    Handlebars.registerHelper("std:text:document", function(text) {
        if(!(text && typeof(text) === "string")) { return ''; }
        return new SafeString($host.renderRTemplate("document_text", text));
    });
    // --------
    ...
    template("std:wait_for_download", function(view) {
        return $host.renderRTemplate("wait_for_download", vString(view,"identifier"), vString(view,"filename"));
    });
    ...
    template("std:oforms:changes", function(view) {
        return $host.renderRTemplate("_client_side_resource", "oforms_changes");
    });
    template("std:jquery_ui:sortable", function(view) {
        return $host.renderRTemplate("_client_side_resource", "jquery_ui");
    });
    template("std:jquery_ui:slider", function(view) {
        return $host.renderRTemplate("_client_side_resource", "jquery_ui_slider");
    });
    ...
    // TODO: Remove if not required for UI redo
    template("std:_platform_client_side_resources", function(view) {
       return $host.renderRTemplate("_platform_client_side_resources");
    });
    // TODO: Remove if not required for UI redo
    template("std:_platform_impersonate_ui", function(view) {
       return $host.renderRTemplate("_platform_impersonate_ui");
    });
