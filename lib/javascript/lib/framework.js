...
function $haplo_framework_initialiser() {
    ...
    this.$registry = {};
    // Because there is a heirarchy of scopes, store the runtime's scope here to make it easy
    // to use from lower scopes.
    this.$registry.$runtimeScope = this;
...
(function() {
    ...
    O.retention = {
        erase: function(x) {
            if(x instanceof $StoreObject) {
                $Ref._eraseObject(x.ref);
            } else if(x instanceof $Ref) {
                $Ref._eraseObject(x);
            } else if(x instanceof $StoredFile) {
                $StoredFile._eraseFile(x);
            } else {
                throw new Error(
                    x ? "O.retention.erase() cannot erase something of this type" : "Must pass something to O.retention.erase()"
                );
            }
        },
        eraseHistory: function(x) {
            if(x instanceof $StoreObject) {
                $Ref._eraseObjectHistory(x.ref);
            } else if(x instanceof $Ref) {
                $Ref._eraseObjectHistory(x);
            } else {
                throw new Error("Must pass a StoreObject or Ref to O.retention.eraseHistory()");
            }
        }
    };
        ...
        var m = function(type) {
            ...
            return function(desc) {
                ...
                if(json) {
                    ...
                    if("groupType" in i) {
                        i.groupType = convertSchemaIdsToRefs(i.groupType);
                    }
                }
                info[desc] = i;
                return i;
    ...
    // TimeZone
    O.timeZone = function(tzName) {
        return $TimeZone.fromName(tzName);
    };
