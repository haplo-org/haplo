...
(function(root) {
    ...
    Handlebars.registerHelper('_internal__indicator_span', function() {
        return new Handlebars.SafeString($registry.standardTemplates["std:_indicator_span"](this));
    });
    ...
    // Support for inclusion within HSVT templates when included from Handlebars templates
    Handlebars.$getFakeTemplateOwner = function() {
        var owner = $registry.$fakeTemplateOwner;
        if(!owner) {
            // Construction to look enough like a Plugin object
            var oclass = function() {};
            oclass.prototype.template = function(n) { return $registry.standardTemplates[n]; };
            owner = $registry.$fakeTemplateOwner = new oclass();
        }
        return owner;
    };
