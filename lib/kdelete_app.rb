...
class KAppDelete
  def self.delete_app(hostname,confirm)
    ...
    KApp.in_application(:no_app) do
      ...
      r = db.exec("SELECT application_id FROM applications WHERE hostname=$1", hostname)
      ...
      r.clear
      # Remove hostnames
      db.perform("DELETE FROM applications WHERE application_id=#{app_id}")
    end
    ...
    KApp.in_application(:no_app) do
      ...
      File.unlink(KObjectStore.get_current_weightings_file_pathname_for_id(app_id))
    end
