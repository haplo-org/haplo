...
module JSFileSupport
  ...
  def self.oFormsFileHTML(stored_file, where)
    if where == 'form'
      ...
      html = %Q!<div class="z__oforms_file"><a href="#{path}"><span class="z__oforms_file_thumbnail">#{thumbnail}</span><span class="z__oforms_file_filename">#{ERB::Util.h(stored_file.presentation_filename)}</span></a>!.dup
      if preview_url
